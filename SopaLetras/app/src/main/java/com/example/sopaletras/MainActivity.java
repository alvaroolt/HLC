...
public class MainActivity extends AppCompatActivity {
    ...
    private final int[] letrasEncontradas = {
    ...
    private void iniciarJuego(){
        ...
        for(int i = 0; i < 10; i++){
            for(int j = 0; j < 10; j++){
                if(partida.getTablero().get(i).get(j).equals("#")){
                    Random numAleatorio = new Random();
                    ImageView img= (ImageView) findViewById(idsImagenes[i][j]);
                    img.setImageResource(letras[numAleatorio.nextInt(26)]);
                }else{
                    ImageView img= (ImageView) findViewById(idsImagenes[i][j]);
                    img.setImageResource(letras[convertirLetraAIndice(partida.getTablero().get(i).get(j))]);
                }
            }
        }
    }
    ...
    private void actualizarVista(){
        for(int i = 0; i < 10; i++){
            for(int j = 0; j < 10; j++){
                if(partida.getTablero().get(i).get(j).length() >1){
                    ImageView img= (ImageView) findViewById(idsImagenes[i][j]);
                    img.setImageResource(letrasEncontradas[convertirLetraAIndice(partida.getTablero().get(i).get(j))]);
                }
            }
        }
    }
